...
import { skillDictStore } from "@/lib/skillDictionaryStore";
...
export function extractHardSkills(text: string, limit = 60): string[] {
  if (!text) return [];
  // Read dictionary fresh each call so admin edits apply immediately.
  const phrases = skillDictStore.getPhrases();
  const singles = skillDictStore.getSingles();
  const singleSet = new Set(singles.map((s) => s.toLowerCase()));
  const phrasesSorted = [...phrases].sort((a, b) => b.length - a.length);
  ...
  const toks = working.match(/[a-z][a-z0-9+.#/-]{1,}/g) || [];
  for (const tok of toks) {
    if (!singleSet.has(tok)) continue;
